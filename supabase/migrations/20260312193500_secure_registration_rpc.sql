-- ── Migration: Enforce Registration Window in RPC ──────────────────
...
BEGIN
    ...
    SELECT status, registered_count, registration_config, reg_start_time, reg_end_time
    INTO v_event_status, v_registered, v_config, v_reg_start, v_reg_end
    ...
    -- 2. Basic guards (Status Check)
    IF v_event_status != 'live' AND v_event_status != 'approved' THEN
        RETURN jsonb_build_object('success', false, 'error', 'Event is not open for registration (Status: ' || v_event_status || ')');
    END IF;

    -- 2.1 Time-based guards (Registration Window)
    IF v_reg_start IS NOT NULL AND now() < v_reg_start THEN
        RETURN jsonb_build_object('success', false, 'error', 'Registration has not started yet');
    END IF;

    IF v_reg_end IS NOT NULL AND now() > v_reg_end THEN
